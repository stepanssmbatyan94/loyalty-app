...
export const updateRewardTranslations = ({
  rewardId,
  translations,
}: UpdateRewardTranslationsDTO): Promise<void> => {
  return api.put(`/rewards/${rewardId}/translations`, { translations });
};
...
export const useUpdateRewardTranslations = ({
  mutationConfig,
}: UseUpdateRewardTranslationsOptions = {}) => {
  ...
  return useMutation({
    onSuccess: (data, variables, ...rest) => {
      queryClient.invalidateQueries({
        queryKey: getRewardTranslationsQueryOptions(variables.rewardId)
          .queryKey,
      });
      onSuccess?.(data, variables, ...rest);
    },
