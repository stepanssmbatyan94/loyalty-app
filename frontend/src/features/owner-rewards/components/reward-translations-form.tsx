...
export function RewardTranslationsForm({
  rewardId,
  onDone,
}: RewardTranslationsFormProps) {
  const t = useTranslations('admin.rewards');
  const { data, isLoading } = useRewardTranslations({ rewardId });
  const updateMutation = useUpdateRewardTranslations({
    mutationConfig: { onSuccess: onDone },
  });
  ...
  const [values, setValues] = useState<Record<string, Record<string, string>>>(
    () => {
      ...
      for (const entry of existing) {
        if (!init[entry.locale])
          init[entry.locale] = { name: '', description: '' };
  ...
  if (isLoading) {
    return (
      <div className="h-32 animate-pulse rounded bg-surface-container-low" />
    );
  }
  ...
  const handleSubmit = () => {
    ...
    for (const locale of SUPPORTED_LOCALES) {
      ...
      if (loc?.name)
        translations.push({ locale, field: 'name', value: loc.name });
      if (loc?.description)
        translations.push({
          locale,
          field: 'description',
    ...
    <div className="space-y-6">
      {SUPPORTED_LOCALES.map((locale) => (
        <div key={locale} className="space-y-3">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-on-background">
              {t('translationDescription', { locale })}
            </label>
            <input
              type="text"
              className="w-full rounded-md border border-outline-variant bg-surface-container-lowest px-3 py-2 text-sm text-on-background focus:outline-none focus:ring-2 focus:ring-primary"
