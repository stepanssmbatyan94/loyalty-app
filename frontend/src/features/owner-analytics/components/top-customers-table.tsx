  ...
  return (
    <div className="rounded-xl border bg-surface-container-lowest shadow-sm">
      <div className="border-b px-6 py-4">
        <h2 className="font-label text-base font-semibold text-on-background">
          {t('topCustomers')}
        </h2>
      </div>
      ...
      {isLoading ? (
        <div className="space-y-2 p-6">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-10 animate-pulse rounded bg-surface-container-low"
            />
          ))}
        </div>
      ) : !data?.data.length ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-left text-on-surface-variant">
                  <th className="px-6 py-3 font-label">{t('rank')}</th>
                  <th className="px-6 py-3 font-label">{t('customer')}</th>
