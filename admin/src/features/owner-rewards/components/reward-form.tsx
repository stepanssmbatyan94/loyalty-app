...
export function RewardForm({ defaultValues, onSubmit, isLoading, onCancel }: RewardFormProps) {
  return (
    <Form
      onSubmit={onSubmit}
      schema={rewardSchema}
    ...
    >
      {({ register, formState }) => (
        <div className="space-y-4">
          ...
          <Input label="Description" error={formState.errors['description']} registration={register('description')} />
          <Input
            type="number"
            label="Points Cost"
          ...
          />
          <Input label="Image URL" error={formState.errors['imageUrl']} registration={register('imageUrl')} />
          <Input
            type="number"
            label="Stock"
