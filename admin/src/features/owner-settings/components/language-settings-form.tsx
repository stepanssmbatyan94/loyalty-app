...
export function LanguageSettingsForm() {
  ...
  return (
    <div className="rounded-xl border bg-surface-container-lowest p-6 shadow-sm">
      <h2 className="mb-4 font-label text-base font-semibold text-on-background">Languages</h2>
      <div className="space-y-4">
        <div>
          <p className="mb-2 text-sm font-medium text-on-background">Supported Locales</p>
          <div className="flex flex-wrap gap-2">
            {AVAILABLE_LOCALES.map((locale) => (
              <button
                ...
                className={cn(
                  'rounded-full px-3 py-1 font-label text-sm transition-colors',
        ...
        <div>
          <p className="mb-2 text-sm font-medium text-on-background">Default Locale</p>
          <select
            value={defaultLocale}
        ...
        <div className="flex justify-end">
          <Button
            ...
            onClick={() => updateMutation.mutate({ supportedLocales: selected, defaultLocale })}
          >
            Save
          </Button>
        </div>
