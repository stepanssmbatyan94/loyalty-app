...
const navItems = [
  ...
  { name: 'Customers', to: '/customers', icon: BarChart3 },
    ...
    <>
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            <item.icon
              className={cn('text-gray-400 group-hover:text-gray-300', 'mr-3 size-5 shrink-0')}
              aria-hidden="true"
            />
            {item.name}
          </Link>
    ...
    <div className="flex min-h-screen w-full flex-col bg-muted/40">
      <aside className="fixed inset-y-0 left-0 z-10 hidden w-60 flex-col border-r bg-gray-900 sm:flex">
        <div className="flex h-16 shrink-0 items-center px-6">
          <span className="font-headline text-lg font-semibold text-white">Beer House Admin</span>
        </div>
      ...
      <div className="flex flex-col sm:pl-60">
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:justify-end sm:border-0 sm:bg-transparent sm:px-6 sm:py-4">
          <Drawer>
            ...
            <DrawerContent side="left" className="bg-gray-900 pt-10 text-white sm:max-w-60">
              <div className="flex h-16 shrink-0 items-center px-6">
